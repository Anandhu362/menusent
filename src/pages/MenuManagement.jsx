        ...
        <header className="h-20 bg-white border-b border-gray-100 px-8 flex items-center justify-between sticky top-0 z-30 shadow-sm">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="flex items-center gap-2 bg-[#ff6b35] text-white px-5 py-2.5 rounded-full text-sm font-bold shadow-md"
            >
        ...
        <div className="p-4 md:p-8 max-w-[1600px] mx-auto pb-12">
          ...
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-lg font-bold text-slate-900">
              All Food Items <span className="text-gray-400 font-medium ml-2">({filteredItems.length})</span>
            </h3>
            ...
            <div className="relative w-64 md:w-80">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#ff6b35] focus:ring-1 focus:ring-[#ff6b35] transition-all shadow-sm font-medium"
              />
              {searchQuery && (
                <button 
          ...
          {filteredItems.length === 0 ? (
             <div className="text-center py-16 bg-white rounded-[24px] border border-gray-100 shadow-sm mt-4">
               ...
               <p className="text-xl font-bold text-slate-700">No food items found</p>
               <p className="text-slate-400 text-sm mt-2 font-medium">Try adjusting your search criteria or add a new product.</p>
             </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredItems.map((item) => {
                ...
                return (
                  <div
                    key={item._id}
                    className={`bg-white rounded-[20px] p-5 shadow-sm border border-gray-100 flex flex-col hover:shadow-md transition-all relative ${!isItemActive ? 'bg-gray-50' : ''}`}
                  >
                    {item.hasVariants && item.variants && item.variants.length > 0 && (
                      <div className="absolute top-3 right-3 bg-orange-100 text-[#ff6b35] text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md z-10">
                        {item.variants.length} Sizes
                      </div>
                    )}

                    <div className="flex items-start gap-4 mb-4">
                      {/* Image container set to relative to hold the absolute Eye button */}
                      <div className="relative">
                        <img
                          src={item.image?.gcsPath}
                          alt={item.name}
                          className={`w-24 h-24 rounded-2xl object-cover border border-gray-100 bg-gray-50 transition-all duration-300 ${!isItemActive ? 'opacity-40 grayscale' : ''}`}
                        />
                        {/* Status Toggle Button */}
                        <button
                          onClick={() => handleToggleActive(item)}
                          className="absolute -top-2 -left-2 bg-white border border-gray-200 p-1.5 rounded-full z-10 hover:bg-gray-100 transition-colors shadow-sm"
                      ...
                      <div className="flex flex-col flex-1 min-w-0">
                        <div className="flex justify-between items-start">
                          <h4 className={`text-lg font-bold truncate pr-6 ${!isItemActive ? 'text-slate-500' : 'text-slate-900'}`}>
                            {item.name}
                          </h4>
                        </div>
                        ...
                        {item.arabicName && (
                          <p
                            dir="rtl"
                            className={`text-sm font-bold leading-tight mb-1 ${!isItemActive ? 'text-orange-600/50' : 'text-orange-600/90'}`}
                          >
                            {item.arabicName}
                          </p>
                        )}
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-gray-50">
                      <button
                        ...
                        className="flex-1 bg-slate-50 text-slate-600 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-100 transition-colors"
                      >
                        Edit
                      </button>
                      <button
