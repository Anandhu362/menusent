...
// ✅ Imports for Settings Panels
import DeliverySettingsPanel from "../components/DeliverySettingsPanel";
import OfferSettingsPanel from "../components/OfferSettingsPanel"; // ✅ NEW: Import Offer Settings Panel
...
const OwnerProfile = () => {
  ...
  // State for the Delivery Settings saving status
  const [savingDelivery, setSavingDelivery] = useState(false);
  
  // ✅ NEW: State for the Offer Settings saving status
  const [savingOffers, setSavingOffers] = useState(false);
  
  const [message, setMessage] = useState({ type: "", text: "" });

  // Holds the full raw restaurant object to pass to the sub-components
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Store the full data object so sub-panels can read their respective settings
        ...
        setProfileData({
          ...
          printerIp: data.printerIp || "192.168.1.220", 
        });
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("printerIp", profileData.printerIp); 
  ...
  // Save Delivery Settings (Map Pin & Rules)
  ...
  // ✅ NEW: Save Promo Code / Offer Settings
  const handleSaveOfferSettings = async (offerData) => {
    setSavingOffers(true);
    try {
      await apiClient.put('/api/restaurants/owner/offers', offerData);
      showMessage("success", "Promo codes and offers saved successfully!");
    } catch (error) {
      showMessage("error", "Failed to save offer settings.");
    } finally {
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                {/* ✅ 3. NEW: Interactive Promo Codes & Offers Component */}
                <OfferSettingsPanel 
                  initialData={restaurantData}
                  onSave={handleSaveOfferSettings}
                  isSaving={savingOffers}
                />
