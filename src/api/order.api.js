...
/**
 * ✅ NEW: Validates a promo code before placing an order.
 * @param {Object} payload - Contains { restaurantId, promoCode, subtotal, phone }
 */
export const validatePromo = async (payload) => {
  const response = await apiClient.post('/api/orders/validate-promo', payload);
  return response.data;
};

/**
 * Calculates the dynamic delivery fee using Google Maps Distance Matrix.
...
/**
 * Public route for comprehensive customer tracking.
...
/**
 * Fetches the time-series data for the Sales Chart Widget.
 * @param {string} restaurantId - The MongoDB ID of the restaurant
...
/**
 * Fetches the active order for a specific table.
 * Used for the Table Management specific order view UI.
 * @param {string} tableId - The MongoDB ID of the table
 */
export const fetchTableActiveOrder = async (tableId) => {
  try {
    const response = await apiClient.get(`/api/orders/table/${tableId}`);
    return response.data;
  } catch (error) {
    console.error(`Error fetching active order for table ${tableId}:`, error);
    throw error;
  }
};